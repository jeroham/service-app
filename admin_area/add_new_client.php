...
if (isset($_POST['company_name'])) {
	...
	$customer1->state = mysql_real_escape_string($_POST['state']);
        $customer1->CheckExisting();
	// See if that product name is an identical match to another product in the system
	$sql = mysql_query("SELECT id FROM cliente WHERE company_name='$company_name' LIMIT 1");
	...
	$sql = mysql_query("INSERT INTO cliente (company_name, contact_first_name, contact_last_name, username, password, phone, cell_phone, email, address1, address2, zip, state, last_log_date) 
        VALUES('$company_name','$contact_first_name','$contact_last_name','$username','$password', '$phone', '$cell_phone', '$email', '$address1', '$address2', '$zip', '$state', now())") or die (mysql_error());
